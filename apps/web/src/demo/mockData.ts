import type { HeaderScanResult, PasswordCheckResult, BreachCheckResult, DomainAuditResult, VulnScanResult } from '@watchpost/shared-types';

export const demoHeaders: HeaderScanResult = {
  url: 'https://example.com',
  headers: {
    'content-security-policy': null,
    'strict-transport-security': 'max-age=31536000; includeSubDomains',
    'x-frame-options': 'DENY',
    'x-content-type-options': 'nosniff',
    'referrer-policy': null,
    'permissions-policy': null,
  },
  score: 43,
  grade: 'D',
  details: [
    { key: 'header.csp',            label: 'Content-Security-Policy',   passed: false },
    { key: 'header.hsts',           label: 'Strict-Transport-Security',  passed: true  },
    { key: 'header.xfo',            label: 'X-Frame-Options',            passed: true  },
    { key: 'header.xcto',           label: 'X-Content-Type-Options',     passed: true  },
    { key: 'header.rp',             label: 'Referrer-Policy',            passed: false },
    { key: 'header.pp',             label: 'Permissions-Policy',         passed: false },
    { key: 'header.https_redirect', label: 'HTTP → HTTPS redirect',      passed: true  },
  ],
};

export const demoPassword: PasswordCheckResult = {
  entropy: 28.5,
  crackTimeSeconds: 42,
  pwnedCount: 3847291,
  score: 17,
  grade: 'F',
  strengthScore: 0,
  warning: 'This is a top-10 password.',
  suggestions: [
    'Add more words that are less common.',
    'Avoid common passwords like "password123".',
    'Use a mix of words, numbers, and symbols.',
  ],
  details: [
    { key: 'pwd.length',    label: 'Length ≥ 12 characters',          passed: false },
    { key: 'pwd.uppercase', label: 'Contains uppercase letters',       passed: false },
    { key: 'pwd.numbers',   label: 'Contains numbers',                 passed: true  },
    { key: 'pwd.special',   label: 'Contains special characters',      passed: false },
    { key: 'pwd.not_pwned', label: 'Not found in known data breaches', passed: false },
    { key: 'pwd.entropy',   label: 'Entropy ≥ 60 bits',               passed: false },
  ],
};

export const demoBreach: BreachCheckResult = {
  email: 'demo@example.com',
  breached: true,
  score: 28,
  grade: 'F',
  riskLevel: 'critical',
  breaches: [
    { name: 'LinkedIn',  date: '2016-05-18', dataTypes: ['Email Addresses', 'Passwords'], passwordRisk: 'hashed',  recordCount: 164611595, domain: 'linkedin.com' },
    { name: 'Adobe',     date: '2013-10-04', dataTypes: ['Email Addresses', 'Passwords', 'Usernames', 'Password Hints'], passwordRisk: 'hashed', recordCount: 152445165, domain: 'adobe.com' },
    { name: 'Dropbox',   date: '2012-07-01', dataTypes: ['Email Addresses', 'Passwords'], passwordRisk: 'hashed',  recordCount: 68648009, domain: 'dropbox.com' },
    { name: 'MySpace',   date: '2008-07-01', dataTypes: ['Email Addresses', 'Passwords', 'Usernames'], passwordRisk: 'plain', recordCount: 359420698, domain: 'myspace.com' },
    { name: 'Collection1', date: '2019-01-07', dataTypes: ['Email Addresses', 'Passwords'], passwordRisk: 'plain', recordCount: 772904991 },
  ],
};

export const demoDomain: DomainAuditResult = {
  domain: 'example.com',
  sslError: null,
  ssl: {
    issuer: "DigiCert Inc",
    validFrom: "2024-01-15T00:00:00.000Z",
    validTo: "2025-04-15T23:59:59.000Z",
    daysUntilExpiry: 18,
    tlsVersion: "TLS 1.2+",
    signatureAlgorithm: "sha256WithRSAEncryption",
    score: 60,
    grade: 'C',
    details: [
      { key: 'ssl.valid',    label: 'Certificate is valid',         passed: true  },
      { key: 'ssl.expiry',   label: 'Expires in more than 30 days', passed: false },
      { key: 'ssl.no_tls10', label: 'TLS 1.0 disabled',             passed: false },
      { key: 'ssl.no_tls11', label: 'TLS 1.1 disabled',             passed: true  },
      { key: 'ssl.sig',      label: 'Strong signature algorithm',    passed: true  },
    ],
  },
  dns: {
    records: {
      A:   ['93.184.216.34'],
      MX:  [],
      NS:  ['a.iana-servers.net', 'b.iana-servers.net'],
      TXT: ['v=spf1 -all'],
    },
    whois: {
      registrar: 'RESERVED-Internet Assigned Numbers Authority',
      createdDate: '1995-08-14T04:00:00Z',
      expiresDate: '2025-08-13T04:00:00Z',
      domainAge: 10748,
    },
    emailSecurity: {
      spf:    'v=spf1 include:_spf.google.com ~all',
      dmarc:  null,
      caa:    [],
      dnssec: false,
      score: {
        score: 33,
        grade: 'F',
        details: [
          { key: 'email.spf_present',   label: 'SPF record present',                        passed: true  },
          { key: 'email.spf_strict',    label: 'SPF policy is strict',                      passed: false },
          { key: 'email.dmarc_present', label: 'DMARC record present',                      passed: false },
          { key: 'email.dmarc_policy',  label: 'DMARC policy is quarantine or reject',      passed: false },
          { key: 'email.caa',           label: 'CAA records restrict certificate issuance', passed: false },
          { key: 'email.dnssec',        label: 'DNSSEC enabled',                            passed: false },
        ],
      },
    },
  },
};

export const demoVuln: VulnScanResult = {
  url: 'https://example.com',
  score: 42,
  grade: 'D',
  findings: [
    { key: 'vuln.server_disclosure', label: 'Server version not exposed',        severity: 'medium', passed: false, detail: 'Apache/2.4.54 (Ubuntu)' },
    { key: 'vuln.powered_by',        label: 'X-Powered-By header absent',        severity: 'low',    passed: false, detail: 'PHP/8.1.12' },
    { key: 'vuln.cors_wildcard',     label: 'CORS not open to all origins',       severity: 'high',   passed: false, detail: 'Access-Control-Allow-Origin: *' },
    { key: 'vuln.csp_unsafe_inline', label: "CSP blocks 'unsafe-inline'",        severity: 'high',   passed: false },
    { key: 'vuln.csp_unsafe_eval',   label: "CSP blocks 'unsafe-eval'",          severity: 'high',   passed: true  },
    { key: 'vuln.csp_wildcard_src',  label: 'CSP has no wildcard source directives', severity: 'medium', passed: true },
    { key: 'vuln.cookie_secure',     label: 'Cookies have Secure flag',          severity: 'medium', passed: false },
    { key: 'vuln.cookie_httponly',   label: 'Cookies have HttpOnly flag',        severity: 'medium', passed: true  },
    { key: 'vuln.cookie_samesite',   label: 'Cookies have SameSite attribute',   severity: 'low',    passed: false },
    { key: 'vuln.robots_sensitive',  label: 'robots.txt does not expose sensitive paths', severity: 'low',    passed: false, detail: '/admin, /backup, /config' },
    { key: 'vuln.security_txt',      label: 'security.txt present',                       severity: 'info',   passed: false },
    { key: 'vuln.mixed_content',     label: 'No mixed content (HTTP resources on HTTPS page)', severity: 'medium', passed: false, detail: 'src="http://images.example.com/logo.png"' },
    { key: 'vuln.sri',              label: 'External scripts and stylesheets use SRI',       severity: 'medium', passed: false, detail: '3 external resources without SRI' },
    { key: 'vuln.meta_generator',   label: 'Meta generator tag does not reveal technology',  severity: 'low',    passed: false, detail: 'WordPress 6.3.2' },
    { key: 'vuln.error_disclosure', label: 'No error messages or stack traces in response',  severity: 'high',   passed: true  },
    { key: 'vuln.no_http_redirect',  label: 'No HTTP URL in redirect chain',                severity: 'medium', passed: false, detail: 'HTTP hop detected in chain (3 steps)' },
    { key: 'vuln.redirect_count',    label: 'Redirect chain has fewer than 4 hops',       severity: 'low',    passed: true  },
    { key: 'vuln.csp_no_default_src', label: 'CSP has a default-src directive',           severity: 'medium', passed: false },
    { key: 'vuln.csp_object_src',    label: 'CSP restricts plugins (object-src)',          severity: 'medium', passed: false },
    { key: 'vuln.csp_base_uri',      label: 'CSP restricts base tag (base-uri)',           severity: 'low',    passed: false },
    { key: 'vuln.port_ftp',      label: 'FTP port (21) not exposed',       severity: 'high', passed: true  },
    { key: 'vuln.port_telnet',   label: 'Telnet port (23) not exposed',    severity: 'high', passed: true  },
    { key: 'vuln.port_mysql',    label: 'MySQL port (3306) not exposed',   severity: 'high', passed: false, detail: 'Port 3306 (MySQL) is publicly reachable' },
    { key: 'vuln.port_redis',    label: 'Redis port (6379) not exposed',   severity: 'high', passed: true  },
    { key: 'vuln.port_mongodb',  label: 'MongoDB port (27017) not exposed', severity: 'high', passed: true  },
  ],
};
