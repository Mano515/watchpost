...
const en: Translations = {
  ...
  checks: {
    ...
    'header.no_xxss':         { label: 'X-XSS-Protection absent or disabled', rec: 'Remove the X-XSS-Protection header or set it to "0".', why: 'The "1" value re-enables an old browser filter with known bypasses that can actually introduce new vulnerabilities.' },
    ...
    'vuln.xxss_protection': { label: 'X-XSS-Protection absent or disabled', rec: 'Remove this header or set it to "0". Never use "1".', why: 'The "1" mode reactivates a deprecated browser feature with known security bypasses that can be weaponized.' },
    ...
    'vuln.no_http_redirect':  { label: 'No HTTP URL in redirect chain', rec: 'Ensure every redirect step uses HTTPS, not HTTP.', why: 'An HTTP hop in the chain exposes request data unencrypted, even if the final destination is HTTPS.' },
    ...
    'vuln.port_ftp':      { label: 'FTP port (21) not exposed',       rec: 'Disable FTP or block port 21 at your firewall. Use SFTP for file transfers.', why: 'FTP sends credentials in plain text — anyone on the network can read them.' },
    'vuln.port_telnet':   { label: 'Telnet port (23) not exposed',    rec: 'Disable Telnet and use SSH for remote administration.', why: 'Telnet transmits everything — including passwords — unencrypted over the network.' },
    'vuln.port_mysql':    { label: 'MySQL port (3306) not exposed',   rec: 'Block port 3306 at your firewall. Databases must never be directly reachable from the internet.', why: 'An exposed database port is one of the most common paths to a full data breach.' },
    'vuln.port_redis':    { label: 'Redis port (6379) not exposed',   rec: 'Block port 6379. Redis has no authentication by default and will accept any command from any client.', why: 'Hundreds of thousands of Redis instances have been wiped or ransomed by automated bots scanning for this port.' },
    'vuln.port_mongodb':  { label: 'MongoDB port (27017) not exposed', rec: 'Block port 27017 at your firewall immediately.', why: 'Exposed MongoDB instances without authentication are trivially readable and writable by anyone.' },
    'vuln.exposed_git':         { label: 'Git repository not publicly accessible', rec: 'Block access to /.git/ in your server config. Nginx: location ~ /\\.git { deny all; }', why: 'An accessible .git/ lets anyone download your entire source code, including API keys, secrets, and commit history.' },
    'vuln.exposed_env':         { label: '.env file not publicly accessible', rec: 'Block .env files at the server level and never deploy them inside the web root.', why: '.env files routinely contain database passwords, API keys, and JWT secrets — the keys to your entire infrastructure.' },
    'vuln.exposed_phpinfo':     { label: 'phpinfo.php not accessible', rec: 'Delete phpinfo.php from production — it was meant for local debugging only.', why: 'phpinfo() dumps your entire server configuration: PHP version, loaded modules, file paths, and environment variables.' },
    'vuln.exposed_wpconfig_bak':{ label: 'wp-config backup not accessible', rec: 'Delete backup copies of wp-config.php and store credentials outside the web root.', why: 'Backup files created by editors or auto-backup plugins often bypass .htaccess rules and expose database credentials.' },
    ...
    'vuln.tls10':               { label: 'TLS 1.0 not accepted', rec: 'Disable TLS 1.0 in your server config. Nginx: ssl_protocols TLSv1.2 TLSv1.3;', why: 'TLS 1.0 is vulnerable to POODLE and BEAST attacks. All major browsers dropped support years ago.' },
    'vuln.tls11':               { label: 'TLS 1.1 not accepted', rec: 'Disable TLS 1.1 and require TLS 1.2 as the minimum version.', why: 'TLS 1.1 uses weaker cipher suites and is deprecated by RFC 8996. It should not be accepted by any modern server.' },
    ...
    'vuln.host_injection':      { label: 'Host header not reflected in redirects', rec: 'Validate the Host header against a strict allowlist before using it in redirects or links.', why: 'If your server uses the Host header to build redirect URLs, attackers can poison password-reset links to point to their own domain.' },
    ...
    'vuln.self_signed_cert':    { label: 'Certificate is not self-signed', rec: "Replace the self-signed certificate with one from a trusted CA (Let's Encrypt is free).", why: 'Browsers display a security warning for self-signed certificates, training users to click through warnings — the most dangerous security habit possible.' },
    ...
    'vuln.source_maps':         { label: 'Source maps not exposed in production', rec: 'Set devtool: false or "hidden-source-map" in webpack/vite so map files are not served publicly.', why: 'Source maps expose your original, unminified source code including comments, variable names, and business logic. They make reverse-engineering trivial.' },
    ...
    'vuln.server_timing':       { label: 'Server-Timing header not exposed', rec: 'Remove the Server-Timing header in production middleware/CDN configuration.', why: 'Server-Timing reveals internal processing times that can enable user-enumeration attacks by measuring whether a username exists based on response speed differences.' },
...
const fr: Translations = {
  ...
  checks: {
    ...
    'header.no_xxss':         { label: 'X-XSS-Protection absent ou désactivé', rec: 'Supprimez l\'en-tête X-XSS-Protection ou mettez-le à « 0 ».', why: 'La valeur « 1 » réactive un ancien filtre navigateur avec des contournements connus qui peuvent créer de nouvelles vulnérabilités.' },
    ...
    'vuln.xxss_protection': { label: 'X-XSS-Protection absent ou désactivé', rec: 'Supprimez cet en-tête ou mettez-le à « 0 ». N\'utilisez jamais « 1 ».', why: 'Le mode « 1 » réactive une fonctionnalité navigateur obsolète avec des contournements connus qui peuvent être exploités.' },
    ...
    'vuln.no_http_redirect':  { label: 'Aucune URL HTTP dans la chaîne de redirections', rec: 'Assurez-vous que chaque étape de redirection utilise HTTPS, pas HTTP.', why: 'Un saut HTTP dans la chaîne expose les données en clair, même si la destination finale est en HTTPS.' },
    ...
    'vuln.port_ftp':      { label: 'Port FTP (21) non exposé',       rec: 'Désactivez FTP ou bloquez le port 21 au pare-feu. Utilisez SFTP pour les transferts de fichiers.', why: 'FTP envoie les identifiants en texte clair — n\'importe qui sur le réseau peut les lire.' },
    'vuln.port_telnet':   { label: 'Port Telnet (23) non exposé',    rec: 'Désactivez Telnet et utilisez SSH pour l\'administration distante.', why: 'Telnet transmet tout — y compris les mots de passe — en clair sur le réseau.' },
    'vuln.port_mysql':    { label: 'Port MySQL (3306) non exposé',   rec: 'Bloquez le port 3306 au pare-feu. Les bases de données ne doivent jamais être accessibles depuis internet.', why: 'Un port de base de données exposé est l\'un des chemins les plus courants vers une violation de données complète.' },
    'vuln.port_redis':    { label: 'Port Redis (6379) non exposé',   rec: 'Bloquez le port 6379. Redis n\'a pas d\'authentification par défaut et accepte toute commande.', why: 'Des centaines de milliers d\'instances Redis ont été vidées ou rançonnées par des bots automatiques.' },
    'vuln.port_mongodb':  { label: 'Port MongoDB (27017) non exposé', rec: 'Bloquez immédiatement le port 27017 au pare-feu.', why: 'Les instances MongoDB exposées sans authentification sont lisibles et modifiables par n\'importe qui.' },
    'vuln.exposed_git':         { label: 'Dépôt Git non accessible publiquement', rec: 'Bloquez l\'accès à /.git/ dans votre config serveur. Nginx : location ~ /\\.git { deny all; }', why: 'Un /.git/ accessible permet à n\'importe qui de télécharger tout votre code source, y compris les clés API et secrets.' },
    'vuln.exposed_env':         { label: 'Fichier .env non accessible publiquement', rec: 'Bloquez les fichiers .env au niveau serveur et ne les déployez jamais dans le répertoire web.', why: 'Les fichiers .env contiennent souvent des mots de passe de BDD, des clés API et des secrets JWT — les clés de toute votre infrastructure.' },
    'vuln.exposed_phpinfo':     { label: 'phpinfo.php non accessible', rec: 'Supprimez phpinfo.php de la production — il n\'est utile qu\'en développement local.', why: 'phpinfo() expose toute la configuration serveur : version PHP, modules chargés, chemins de fichiers et variables d\'environnement.' },
    'vuln.exposed_wpconfig_bak':{ label: 'Sauvegarde wp-config non accessible', rec: 'Supprimez les copies de sauvegarde de wp-config.php et stockez les identifiants hors du répertoire web.', why: 'Les fichiers de sauvegarde créés par des éditeurs ou plugins contournent souvent les règles .htaccess et exposent les identifiants de BDD.' },
    ...
    'vuln.tls10':               { label: 'TLS 1.0 non accepté', rec: 'Désactivez TLS 1.0 dans votre config serveur. Nginx : ssl_protocols TLSv1.2 TLSv1.3;', why: 'TLS 1.0 est vulnérable aux attaques POODLE et BEAST. Tous les navigateurs majeurs ont abandonné son support.' },
    'vuln.tls11':               { label: 'TLS 1.1 non accepté', rec: 'Désactivez TLS 1.1 et exigez TLS 1.2 comme version minimale.', why: 'TLS 1.1 utilise des suites de chiffrement faibles et est déprécié par la RFC 8996. Il ne devrait plus être accepté.' },
    ...
    'vuln.host_injection':      { label: 'L\'en-tête Host n\'est pas reflété dans les redirections', rec: 'Validez l\'en-tête Host par rapport à une liste d\'autorisation stricte avant de l\'utiliser dans des redirections.', why: 'Si votre serveur utilise l\'en-tête Host pour construire des URL de redirection, des attaquants peuvent empoisonner les liens de réinitialisation de mot de passe.' },
    ...
    'vuln.self_signed_cert':    { label: 'Le certificat n\'est pas auto-signé', rec: 'Remplacez le certificat auto-signé par un certificat d\'une CA de confiance (Let\'s Encrypt est gratuit).', why: 'Les navigateurs affichent un avertissement de sécurité pour les certificats auto-signés, habituant les utilisateurs à ignorer les alertes — la pire habitude de sécurité possible.' },
    ...
    'vuln.source_maps':         { label: 'Source maps non exposées en production', rec: 'Configurez devtool: false ou "hidden-source-map" dans webpack/vite pour ne pas servir les fichiers .map publiquement.', why: 'Les source maps exposent votre code source original non minifié incluant commentaires, noms de variables et logique métier, rendant le reverse-engineering trivial.' },
...
const es: Translations = {
  ...
  checks: {
    ...
    'header.no_xxss':         { label: 'X-XSS-Protection ausente o desactivado', rec: 'Elimina la cabecera X-XSS-Protection o ponla a "0".', why: 'El valor "1" reactiva un filtro de navegador antiguo con bypasses conocidos que puede introducir nuevas vulnerabilidades.' },
    ...
    'vuln.xxss_protection': { label: 'X-XSS-Protection ausente o desactivado', rec: 'Elimina esta cabecera o ponla a "0". Nunca uses "1".', why: 'El modo "1" reactiva una función de navegador obsoleta con bypasses conocidos que pueden ser explotados.' },
    ...
    'vuln.no_http_redirect':  { label: 'Sin URL HTTP en la cadena de redirecciones', rec: 'Asegúrate de que cada paso de redirección use HTTPS, no HTTP.', why: 'Un salto HTTP en la cadena expone los datos en texto plano, aunque el destino final sea HTTPS.' },
    ...
    'vuln.port_ftp':      { label: 'Puerto FTP (21) no expuesto',       rec: 'Deshabilita FTP o bloquea el puerto 21 en tu firewall. Usa SFTP para transferencias.', why: 'FTP envía las credenciales en texto plano — cualquiera en la red puede leerlas.' },
    'vuln.port_telnet':   { label: 'Puerto Telnet (23) no expuesto',    rec: 'Deshabilita Telnet y usa SSH para administración remota.', why: 'Telnet transmite todo — incluidas las contraseñas — sin cifrar por la red.' },
    'vuln.port_mysql':    { label: 'Puerto MySQL (3306) no expuesto',   rec: 'Bloquea el puerto 3306 en tu firewall. Las bases de datos nunca deben ser accesibles desde internet.', why: 'Un puerto de base de datos expuesto es uno de los caminos más comunes hacia una brecha de datos completa.' },
    'vuln.port_redis':    { label: 'Puerto Redis (6379) no expuesto',   rec: 'Bloquea el puerto 6379. Redis no tiene autenticación por defecto y acepta cualquier comando.', why: 'Cientos de miles de instancias Redis han sido vaciadas o secuestradas por bots automatizados.' },
    'vuln.port_mongodb':  { label: 'Puerto MongoDB (27017) no expuesto', rec: 'Bloquea el puerto 27017 en tu firewall inmediatamente.', why: 'Las instancias MongoDB expuestas sin autenticación son trivialmente legibles y modificables por cualquiera.' },
    'vuln.exposed_git':         { label: 'Repositorio Git no accesible públicamente', rec: 'Bloquea el acceso a /.git/ en tu config del servidor. Nginx: location ~ /\\.git { deny all; }', why: 'Un /.git/ accesible permite a cualquiera descargar todo tu código fuente, incluyendo claves API y secretos.' },
    'vuln.exposed_env':         { label: 'Archivo .env no accesible públicamente', rec: 'Bloquea los archivos .env a nivel de servidor y nunca los despliegues en el directorio web.', why: 'Los archivos .env suelen contener contraseñas de BD, claves API y secretos JWT — las llaves de toda tu infraestructura.' },
    'vuln.exposed_phpinfo':     { label: 'phpinfo.php no accesible', rec: 'Elimina phpinfo.php de producción — solo es útil en desarrollo local.', why: 'phpinfo() expone toda la configuración del servidor: versión de PHP, módulos cargados, rutas de archivos y variables de entorno.' },
    'vuln.exposed_wpconfig_bak':{ label: 'Copia de seguridad de wp-config no accesible', rec: 'Elimina las copias de seguridad de wp-config.php y almacena las credenciales fuera del directorio web.', why: 'Los archivos de copia de seguridad creados por editores o plugins suelen eludir las reglas .htaccess y exponen credenciales de BD.' },
    ...
    'vuln.tls10':               { label: 'TLS 1.0 no aceptado', rec: 'Deshabilita TLS 1.0 en tu config del servidor. Nginx: ssl_protocols TLSv1.2 TLSv1.3;', why: 'TLS 1.0 es vulnerable a los ataques POODLE y BEAST. Todos los navegadores principales han abandonado su soporte.' },
    'vuln.tls11':               { label: 'TLS 1.1 no aceptado', rec: 'Deshabilita TLS 1.1 y exige TLS 1.2 como versión mínima.', why: 'TLS 1.1 usa suites de cifrado débiles y está obsoleto según RFC 8996. No debería ser aceptado por ningún servidor moderno.' },
    ...
    'vuln.self_signed_cert':    { label: 'El certificado no es autofirmado', rec: "Reemplaza el certificado autofirmado por uno de una CA de confianza (Let's Encrypt es gratuito).", why: 'Los navegadores muestran una advertencia de seguridad para los certificados autofirmados, entrenando a los usuarios a ignorar las alertas — el peor hábito de seguridad posible.' },
    ...
    'vuln.source_maps':         { label: 'Source maps no expuestos en producción', rec: 'Configura devtool: false o "hidden-source-map" en webpack/vite para no servir archivos .map públicamente.', why: 'Los source maps exponen tu código fuente original sin minificar incluyendo comentarios, nombres de variables y lógica de negocio, haciendo trivial la ingeniería inversa.' },
